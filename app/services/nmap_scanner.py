...
def perform_nmap_scan(scan_id: str, target: str, scan_type: ScanType):
    with SessionLocal() as db:
        try:
            ...
            host = list(result["scan"].keys())[0] if "scan" in result and result["scan"] else target
            host_data = result["scan"][host] if "scan" in result and host in result["scan"] else None
            ...
            command = nm.command_line() if hasattr(nm, 'command_line') else ""

            update_scan(db, scan_id, {
            ...
            update_scan(db, scan_id, {"status": ScanStatus.failed.value})


async def run_nmap_scan(scan_id: str, target: str, scan_type: ScanType):
    with SessionLocal() as db:
        update_scan(db, scan_id, {"status": ScanStatus.running.value})
